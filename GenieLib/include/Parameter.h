...
namespace GenICam {
	...
	class StandardFeature : public Feature {
	public:
		...
		void GainAuto(bool value) {
			SetValue("GainAuto", (int)(value ? 1 : 0));
		}

		bool GainAuto() {
			return (GetValueInteger("GainAuto") == 1) ? true : false;
		}
		...
		int Height() {
			return GetValueInteger("Height");
		}

		void TriggerSource(TriggerSourceType type) {
			SetValue("TriggerSource", type);
		}

		TriggerSourceType TriggerSource() {
			return  (TriggerSourceType)GetValueInteger("TriggerSource");
		}
	};
}
