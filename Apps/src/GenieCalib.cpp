...
int main(int argc, char* argv[])
{
    ...
    for(;;)
    {
		...
		if (key == 'd' && mode == CALIBRATED && found) {
			//Construct the chessboard model
			std::vector<cv::Point3d> objectPoints;
			for (int i = 0; i < s.boardSize.height; i++) {
				for (int j = 0; j < s.boardSize.width; j++) {
					objectPoints.push_back(
						cv::Point3d(double(j * s.squareSize), float(i * s.squareSize), 0));
				}
			}

			cv::Mat rvec, tvec;
			cv::solvePnP(objectPoints, pointBuf, cameraMatrix, distCoeffs, rvec, tvec);

			cerr << "t: " << tvec << ", r: " << rvec << endl;
		}
